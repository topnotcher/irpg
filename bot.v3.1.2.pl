...
sub penalize {
	...
	my $dstnick = substr($newnick,1);
	my $srcnick = $rps{$username}{nick};
        $rps{$username}{nick} = substr($newnick,1);

	if ( is_guest_nick($dstnick) or is_guest_nick($srcnick) ) {
		notice(
			"You have not been penalized for switching to/from a Guest nick.", 
			$rps{$username}{nick}
		);
	} else {
